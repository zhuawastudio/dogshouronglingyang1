            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">犬收藏展示</span>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../zaikuquan/detail.jsp?id='+item.zaikuquanId)">
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../zaikuquan/detail.jsp?id='+item.zaikuquanId)"><span>{{item.zaikuquanName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.newMoney"><span style="color: red;">{{item.newMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteZaikuquanCollection(item.id)">删除 {{item.zaikuquanName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 zaikuquanCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            zaikuquanCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteZaikuquanCollection(id){
                var paramArray = [];
                paramArray.push(id);
                layui.http.requestJson(`zaikuquanCollection/delete`, 'post',paramArray, function (res) {
                    if (res.code == 0) {
                        layer.msg('删除成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery'], function() {
        ...
        function pageList() {
            ...
            // 当前表
                                // 获取列表数据
            http.request('zaikuquanCollection/list', 'get', param, function(res) {
